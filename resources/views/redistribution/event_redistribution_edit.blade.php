...
@section('content')
    <div class="container">
        <div class="card-header">
            <h5 class="mb-2">@if($event) Edit @else Create @endif Event for Redistribution</h5>
        </div>
        <div class="card-body">
            <form action="@if($event) {{ route('events.update', $event->id) }} @else {{ route('events.store') }} @endif"
                method="post" class="needs-validation mb-3" novalidate>
                @csrf
                @if($event)
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="event_name" name="event_name"
                            value="{{ $event ? $event->event_name : old('event_name') }}" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group row">
                            <input type="datetime-local" class="form-control" id="event_date" name="event_date"
                                value="{{ $event ? date('Y-m-d\TH:i', strtotime($event->event_date)) : old('event_date') }}"
                                required>
                ...
                <div class="mb-3">
                    <iframe id="google-map-iframe" width="100%" height="300" allowfullscreen scrolling="no"
                        class="mb-2"></iframe>
                    ...
                    <input type="text" id="location" name="location" class="form-control"
                        value="{{ $event ? $event->location : old('location') }}" placeholder="Enter your event location">
                    @error('location')
                        <div class="note">{{ $message }}</div>
                    @enderror
                </div>
                <div class="row mb-3">
                    ...
                    <div class="input-group col-md-12">
                        <input type="number" step="any" class="form-control" id="people_quantity" name="people_quantity"
                            value="{{ $event ? $event->people_quantity : old('people_quantity') }}" required>
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="leftovers_description" name="leftovers_description"
                        rows="3">{{ $event ? $event->leftovers_description : old('leftovers_description') }}</textarea>
                ...
                <div class="text-center">
                    <button type="submit" class="btn btn-primary">@if($event) Update @else Save @endif</button>
                </div>
            </form>
        </div>
    </div>
