...
const donatedAmount = @json($totalsum);
...
const chart = new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [{
            label: 'Latest 10 Day Donation Amount',
            data: donatedAmount,
            borderColor: '#32A660',
            borderWidth: 1,
            fill: false
  ...
  <div class="row">
    <div class="col-lg-6">
      <div class="text-center">
        <div>
          <h5><b>Total Donors</b></h5>
        </div>

        <div style="color:#4caf50;">
          <h1><b>{{ $record->donations_count }}</b></h1>
        </div>
      </div>
    </div>
    <div class="col-lg-6">
      <div class="text-center">
        <h5><b>Raised Fund</b></h5>
      </div>

      <canvas id="chart" style="min-height: 100px; height: 100px; max-height: 100px; max-width: 100%;"></canvas>
      <div class="text-center">
        <h3>{{ $donated }} %</h3>
      </div>

      <div class="text-center">
        <h3>{{number_format($record -> donations_sum_donation_amount,2) ?? 0.00}} / {{number_format($record -> target_goal,2)}}</h3>
      </div>
    </div>
  </div>
